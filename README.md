...
2. **Добавьте аргумент дебаггера Яндекс.Музыке**
   - Найдите ярлык, через который вы запускаете приложение Яндекс.Музыки
   - Откройте свойства ярлыка
   - В строке `Объект`, после пути, через пробел добавьте аргумент `--remote-debugging-port=9228` (порт можно заменить в настройках данного приложения, `9228` - стандартный)
